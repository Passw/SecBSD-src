# hints/secbsd.sh
#
# hints file for SecBSD; Todd Miller <millert@openbsd.org>
# Edited to allow Configure command-line overrides by
#  Andy Dougherty <doughera@lafayette.edu>
#
# To build with distribution paths, use:
#	./Configure -des -Dsecbsd_distribution=defined
#

# SecBSD has a better malloc than perl
...
# SecBSD doesn't need libcrypt but many folks keep a stub lib
# around for old NetBSD binaries.
libswanted=`echo $libswanted | sed 's/ crypt / /'`

# SecBSD hasn't ever needed linking to libutil
...
case "$usethreads" in
$define|true|[yY]*)
	# any secbsd version dependencies with pthreads?
...
# When building in the SecBSD tree we use different paths
# This is only part of the story, the rest comes from config.over
case "$secbsd_distribution" in
''|$undef|false) ;;
*)
